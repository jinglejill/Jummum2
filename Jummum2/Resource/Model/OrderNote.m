...
+(NSString *)getNoteIDListInTextWithOrderTakingID:(NSInteger)orderTakingID branchID:(NSInteger)branchID
{
    int i=0;
    NSString *strNote = @"";
    NSMutableArray *noteList = [OrderNote getNoteListWithOrderTakingID:orderTakingID branchID:branchID];
    if([noteList count] > 0)
    {
        for(Note *item in noteList)
        {
...
{
    float sum = 0;
    NSMutableArray *orderNoteList = [OrderNote getOrderNoteListWithOrderTakingID:orderTakingID];
    for(OrderNote *item in orderNoteList)
    {
        Note *note = [Note getNote:item.noteID branchID:branchID];
        sum += item.quantity * note.price;
    }
    return sum;
}
